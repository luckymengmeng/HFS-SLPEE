# -*- coding: UTF-8 -*- 
'''
@Time       :   2020/1/13 4:54 下午
@Author     :   Meng Yajie
@FileName   :   1_main_luad.py
@Software   :   PyCharm
'''
import load
import os
...
# 设置目录
data_Dir = "../../dataset/TCGA_LUAD/TCGA_LUAD_logFC3_logFC0.5_20/methyGeneMerge/"
# 数据，离散格式，行是样本，列为特征
...
for num_fea in range(12,13,1):
    ...
    for (train_index, test_index),foldNumber in zip(skf.split(X, y),range(n)):
        ...
        lr = xgb.XGBClassifier(max_depth=6, n_estimators=100, num_round=5,random_state=RANDOM_SEED, objective= "binary:logistic", eval_metric= "error")

        # 设置use_probas =
        # True，第一层单个模型预测结果是类别概率，而不是label
