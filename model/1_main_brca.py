# -*- coding: UTF-8 -*- 
'''
@Time       :   2020/1/13 3:35 下午
@Author     :   Meng Yajie
@FileName   :   1_main_brca.py
@Software   :   PyCharm
'''
import load
import os
...
# 设置目录
data_Dir = "../../dataset/TCGA_BRCA/TCGA_BRCA_logFC3_logFC0.5_18/methyGeneMerge/"
# 数据，离散格式，行是样本，列为特征
...
for num_fea in range(21,22,1):
    ...
    tprs = []
    aucs = []
    # 等差数列,此时100是指X的样本数 X.shape[0]
    # mean_fpr = np.linspace(0, 1, 100) #此处的100应修改
    mean_fpr = np.linspace(0, 1, X.shape[0])
    y_true = []
    y_score = []
    ...
    for (train_index, test_index),foldNumber in zip(skf.split(X, y),range(n)):
        ...
        # AUC
        probas_ = grid.predict_proba(X_test_scale_raw)
        y_true.append(y_test_raw)
        y_score.append(probas_[:, 1])
        # Compute ROC curve and area the curve
        # fpr, tpr, thresholds = roc_curve(y_test_raw, probas_[:, 1])
    ...
    # 画该维度的AUC曲线
    # alpha是控制透明度，linewidth or lw 是控制线条的宽度
    # 画对角线
    ...
    plt.ylim([0.96, 1.0])
    # plt.show()
    plt.savefig("AUPR.svg", bbox_inches="tight")
